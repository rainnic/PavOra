...
function testShowNewMonths() {
  ...
  var end = Date.now();
  Logger.log(
    "Tempo di esecuzione funzione: " + (end - start) / 1000 + " secondi"
  );
}
  ...
  try {
    ...
    if (eventi.length != 0) {
      ...
      if (
        ...
        freeStructures = freeStructures.filter((s) =>
          selectionData.includes(s[0])
        ); // s[0] = nome struttura
      }
...
function specialNewDailyEvent() {
  ...
  if (selectionResult) {
    ...
    */
    showFreeDailyHall(
      selectionResult.firstDate,
      selectionResult.lastDate,
      selectionResult.firstColumnData
    );
  ...
  for (let i = 0; i < firstColumnValues.length; i++) {
    if (findKey(firstColumnValues[i], strutture(), 6) > -1) {
      convertedValues.push(
        strutture()[findKey(firstColumnValues[i], strutture(), 6)][0]
      );
    }
  }
...
function specialUpdateEvent(first, last) {
  ...
  // PASSA ANCHE first e last alla funzione
  getCellNote(first, last, id);
  highlightCellsWithIdM(id);

  // IMPORTANTE: ritorna un valore per il successHandler
  return true;
}
...
function getCellSpecialNote(what, first, last) {
  ...
  */
  ...
  if (nameNote.length != 0) {
    var note =
      extractRegex(regexId, nameNote) != 0
        ? extractRegex(regexId, nameNote)
  ...
  if (note && what === "deleteEvent") {
    ...
    ui.alert(
      "Pronto a cancellare con questi dati: " +
        note +
        " | " +
        first +
        " | " +
...
function showMonths(first, last, structures, keyword, period) {
  try {
    ...
    if (!structures) {
      structures = struttureBigKey(
        readVariables("struttureScelteSup", DataStructures)
      ).join(",");
    }

    const eventi = events2Array(
      convertDateBar(firstDate[0]),
      convertDateBar(lastDate[2]),
      categories()[0][0],
      keyword
    );

    var matrici = generaMatriceEventiConColori(
      eventi,
      formatDateMaster(firstDate[0]).dataXweb,
      formatDateMaster(lastDate[2]).dataXweb,
      structures
    );
    //const sh = SpreadsheetApp.getActiveSpreadsheet().getActiveSheet();
    popolaFoglioSheet(
      matrici.matriceOutput,
      matrici.matriceNoteOutput,
      matrici.matriceColorOutput,
      matrici.matriceDomenicheOutput
    );
    checkToday(sh);
...
function resetFoglioConNuovo() {
  ...
  foglioNuovo
    .getRange(1, 1)
    .setValue(
      translate("specialEvent.newSheet") +
        "\n" +
        formatDateMaster(today).ora +
        " " +
        formatDateMaster(today).giorno
    );
  //protectSheets(); //Commented out due to permission errors encountered by the writer and reader user roles
  return foglioNuovo;
}

function generaMatriceEventiConColori(
  eventi,
  intervalloInizio,
  intervalloFine,
  luoghi
  ...
  for (const struttura of struttureCache) {
    ...
    const figli = struttura[18]
      ? struttura[18].split(",").map((s) => s.trim())
      : [];
  ...
  for (const evento of eventi) {
    ...
    for (const luogoRichiesto of luoghiArray) {
      ...
      if (coinvolto) {
        ...
        for (
          let t = dataInizio.getTime();
          t <= dataFine.getTime();
          t += millisecondsPerDay
        ) {
          const giorno = new Date(t);
          const giornoStr = giorno.toISOString().split("T")[0];
  ...
  for (const giorno of dateList) {
    ...
    intestazioneMesi.push(
      giorno.getDate() === 1
        ? nomiMesi[giorno.getMonth()] + "\n" + giorno.getFullYear()
        : ""
    );
  ...
  for (let i = 0; i < struttureCache.length; i++) {
    struttureLookup.set(struttureCache[i][0], {
      index: i,
      nome: struttureCache[i][6],
    });
  }
  ...
  for (const giorno of dateList) {
    rigaMeseSoloPrimi.push(
      giorno.getDate() === 1 ? nomiMesi[giorno.getMonth()] : ""
    );
  }
        ...
        if (regexE.test(evento.note) || regexL.test(evento.note)) {
          ...
          for (const numero of evento.numeriUnici) {
            if (
              !eventiEL.has(numero) ||
              new Date(giornoStr) <
                new Date(eventiEL.get(numero).primaOccorrenza)
            ) {
...
function popolaFoglioSheet(
  ...
  if (righe > 0 && colonne > 0) {
    ...
    if (matriceColorOutput && matriceColorOutput.length > 0) {
      sheet
        .getRange(4, 1, matriceColorOutput.length, matriceColorOutput[0].length)
  ...
  // Intestazione
  sheet
    .getRange(4, 1, 1, colonne)
    .setFontWeight("bold")
    .setHorizontalAlignment("left");

  sheet
    .getRange(5, 1, 1, colonne)
  ...
  if (matriceDomenicheOutput && matriceDomenicheOutput.length > 0) {
    ...
    for (let col = 1; col < rigaDomeniche.length; col++) {
      if (rigaDomeniche[col] === true) {
        rangesDomenica.push(
          sheet.getRange(4, col + 1, matriceOutput.length, 1)
        );
      }
    }
  ...
  sheet
    .getRange(lr + 1, 1)
    .setValue(translate("specialEvent.confirmed"))
    .setFontSize(8)
    .setFontWeight("bold")
    .setBackground(colori[1][0])
    .setHorizontalAlignment("center");
  sheet
    .getRange(lr + 2, 1)
    .setValue(translate("specialEvent.optionated"))
    .setFontSize(8)
    .setFontWeight("bold")
    .setBackground(colori[4][0])
    .setHorizontalAlignment("center");
  sheet
    .getRange(lr + 3, 1)
    .setValue(translate("specialEvent.offer"))
    .setFontSize(8)
    .setFontWeight("bold")
    .setBackground(colori[14][0])
    .setHorizontalAlignment("center");
  sheet
    .getRange(lr + 4, 1)
    .setValue(translate("specialEvent.work"))
    .setFontSize(8)
    .setFontWeight("bold")
    .setBackground(colori[15][0])
    .setHorizontalAlignment("center");
  sheet
    .getRange(lr + 5, 1)
    .setValue(translate("specialEvent.concurrent"))
    .setFontSize(8)
    .setFontWeight("bold")
    .setBackground(colori[6][0])
    .setHorizontalAlignment("center");
